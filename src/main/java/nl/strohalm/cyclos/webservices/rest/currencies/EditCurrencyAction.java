 ...
 * @author luis
 */
public class EditCurrencyAction extends BaseFormAction implements
	...
	protected ActionForward handleDisplay(final ActionContext context)
		...
		boolean editable = permissionService
				.hasPermission(AdminSystemPermission.CURRENCIES_MANAGE);
		Currency currency;
		if (isInsert) {
		...
		request.setAttribute("pendingRateInitProgression",
	...
	protected ActionForward handleSubmit(final ActionContext context)
		...
		return ActionHelper.redirectWithParam(context.getRequest(),
				context.getSuccessForward(), "currencyId", currency.getId());
	}

	@Override
	protected void validateForm(final ActionContext context) {
		final EditCurrencyForm form = context.getForm();
		final Currency currency = getDataBinder().readFromString(
				form.getCurrency());
		final WhatRate whatRate = new WhatRate();
		whatRate.setaRate(form.isEnableARate());
		whatRate.setdRate(form.isEnableDRate());
		// Skip the validation for i-rate here, as it is done on the
		// currencyService.save method anyways.
