...
public class EditGroupFilterAction extends BaseFormAction {
	...
	protected ActionForward handleSubmit(final ActionContext context)
		...
		context.sendMessage(isInsert ? "groupFilter.inserted"
				: "groupFilter.modified");
	...
	protected void prepareForm(final ActionContext context) throws Exception {
		...
		if (!isInsert) {
			final GroupFilter groupFilter = groupFilterService.load(id,
					...
					GroupFilter.Relationships.VIEWABLE_BY,
			...
			request.setAttribute("customizedFiles",
					customizedFileService.search(cfQuery));
		}
		...
		final Collection<MemberGroup> groups = (Collection<MemberGroup>) groupService
				.search(query);
		...
		request.setAttribute("canManageCustomizedFiles",
				customizedFileService.canViewOrManageInGroupFilters());
	}

	@Override
	protected void validateForm(final ActionContext context) {
		final EditGroupFilterForm form = context.getForm();
		final GroupFilter groupFilter = getDataBinder().readFromString(
				form.getGroupFilter());
