...
public class ConfirmLoanController extends BaseRestController {
	...
	protected ConfirmLoanResponseDto handleSubmit(
		...
		context.sendMessage(pending ? "loan.awaitingAuthorization"
				: "loan.granted");
		if (form.getMemberId() > 0) {
			return ActionHelper.redirectWithParam(context.getRequest(),
					context.findForward("member"), "memberId",
					form.getMemberId());
		} else {
			return ActionHelper.redirectWithParam(context.getRequest(),
					context.findForward("loanGroup"), "loanGroupId",
					form.getLoanGroupId());
		}
	}
	...
	protected void prepareForm(final ActionContext context) throws Exception {
		...
		final TransactionFeePreviewDTO preview = transactionFeeService.preview(
				context.getAccountOwner(), member, transferType, amount);
		...
		request.setAttribute("wouldRequireAuthorization",
				paymentService.wouldRequireAuthorization(payment));
	}
	...
	private GrantLoanDTO validateLoan(final ConfirmLoanRequestDto context) {
		final GrantLoanDTO payment = (GrantLoanDTO) context.getSession()
				.getAttribute("loan");
		if (payment == null) {
